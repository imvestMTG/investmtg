...
| `components/Header.js` | persistent | 3-tier header: main header (logo + Scryfall autocomplete search + Sell CTA + cart badge + theme toggle + user avatar/dropdown + mobile hamburger) + nav row with dropdown groups (Market Data, My Tools, Community) + direct links (Search, Portfolio, Sell). Mobile: full-screen slide-out nav. Search dispatches `investmtg-search` custom event. CSS uses `hdr-*` namespace. |
