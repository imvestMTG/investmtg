...
`sw.js` is on cache version `investmtg-v103`. The caching strategy is:
...
| `components/ScannerView.js` | `#scan` | Card scanner v2: dual-strategy OCR using Tesseract.js. Crops top 18% for card name + bottom 30% for collector number. Adaptive threshold preprocessing with integral-image-based local averaging. Scryfall lookup priority: collector+set > collector-only > autocomplete fuzzy name > search. Supports camera capture and photo upload. Strategy badge shows match method. Session scan history (in-memory). |
